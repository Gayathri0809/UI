...
function Cart() {
  ...
  const loadLists = async () => {
    const result = await axios.get("http://localhost:4830/addres");
    setLists(result.data); 
  };
  const deleteItem = async (id) => {
    await axios.delete(`http://localhost:4830/address/${id}`);
    loadLists();
  };
  const removeItem = async (id) => {
    await axios.delete(`http://localhost:4830/history/${id}`);
    loadList();
  };
  useEffect(() => {
    loadList();
